...
import type { TableTrasfarableType } from '@shared/utils/dtos/api/response/table-response.interface';
import type { Table__entity__(pascalCase)DtoType } from '@__module__(camelCase)/application/dto/__entity__(kebabCase)/table-__entity__(kebabCase).dto';
...
export class Table__entity__(pascalCase)UseCase {
  ...
  public async handle(dto: Table__entity__(pascalCase)DtoType) {
    try {
      ...
      const response: TableTrasfarableType = this.generateResponse(
        count,
        entites,
        elPerPage,
        currentPage,
  ...
  private convertToTableData(entity: __entity__(pascalCase)Type): __entity__(pascalCase)Entity {
    const result = new __entity__(pascalCase)Entity(entity)
    return result;
  }

  private getFilters(
    filters: Omit<Table__entity__(pascalCase)DtoType, 'elPerPage' | 'currentPage'>,
